...
async def test_get_daily_prompt_returns_matching_row(async_session: AsyncSession) -> None:
    async with async_session.begin():
        async_session.add(DailyPrompt(day_identifier=5, content="Day 5 content."))

    result = await get_daily_prompt(async_session, 5)
    assert result is not None
    assert result.day_identifier == 5
...
async def test_get_daily_prompt_wrong_identifier_returns_none(async_session: AsyncSession) -> None:
    async with async_session.begin():
        async_session.add(DailyPrompt(day_identifier=3, content="Day 3."))
...
@pytest.mark.asyncio
async def test_day_identifier_is_unique(async_session: AsyncSession) -> None:
    from sqlalchemy.exc import IntegrityError

    async with async_session.begin():
        async_session.add(DailyPrompt(day_identifier=7, content="First."))

    with pytest.raises(IntegrityError):
        async with async_session.begin():
            async_session.add(DailyPrompt(day_identifier=7, content="Duplicate."))
