...
async def console_root(_: None = Depends(require_admin)) -> HTMLResponse:
    ...
    <html lang="en">
      <head>
        ...
        <style>
          :root{{color-scheme:light;--bg:#f6f3ef;--panel:#fff;--text:#1f2328;--muted:#5f6b7a;--accent:#1c5d99;--border:#e5e1da}}
          *{{box-sizing:border-box}}
          body{{margin:0;font-family:"SF Pro Text","Segoe UI","Helvetica Neue","Noto Sans",sans-serif;color:var(--text);background:radial-gradient(1200px 600px at 10% -10%,#fff8ee 0%,var(--bg) 55%,#f0f4f7 100%)}}
          ...
          .card{{display:block;padding:16px;border:1px solid var(--border);border-radius:14px;text-decoration:none;color:inherit;background:var(--panel);transition:transform .12s ease,box-shadow .12s ease,border-color .12s ease}}
          .card:hover{{transform:translateY(-2px);border-color:#d8d2c6;box-shadow:0 12px 28px rgba(31,35,40,.12)}}
